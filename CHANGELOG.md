...
### Added
- `quick_preview_signal_intensity.R` — plots raw per-well signal intensity in plate format directly from a SpectraMax SDA export (`.csv`/`.xls`/`.txt`) or a generic long/plate `.xlsx`, with no `exp_metadata`/`assay_metadata` needed. One facet per wavelength, or per excitation/emission pair for a SpectraMax fluorescence export with multiple channels (reads the excitation wavelength at row 2/col 21 alongside the emission wavelength the main engines already read from col 17). Also exports the tidied values as long- and wide-format CSVs. Ported from Daniel's own `platextractR` scripts, reusing this repo's already-hardened SpectraMax and generic-Excel parsers rather than those scripts' own simpler ones. See CLAUDE.md for a real long-format join bug caught and fixed while building it.
- Suppressed wells (`suppress = TRUE`) are now marked directly on the signal-heatmap plot (`p_plate_signal`), in all three engines, with a red strikethrough (`geom_segment`, spanning `col ± 0.35` across the well circle) — in addition to the existing red-×-plus-label marking on the sample raw-signal scatter (`p_sample_raw`). Ported from the sibling `process_spectramax_ABTS_kinetics.R` script's AUC-heatmap strikethrough convention.
- `p_plate_signal` well labels for absorbance reads (`assay_metadata$read_type` matching `"absorbance"`, case-insensitive) now show fixed 3-decimal values (0.001 precision, `sprintf("%.3f", signal)`) instead of the 3-significant-figure auto-scaled labels used for fluorescence/luminescence reads. Display-only — the underlying `signal` and every downstream calculation are unaffected either way.

### Fixed
- `process_spectramax_endpoint_std_curve.R`, `process_synergy2_endpoint_std_curve.R`, `process_generic_excel_endpoint_std_curve.R`: `conflicts_prefer(here::here, dplyr::filter, ggplot2::annotate)` silently broke every `annotate("text_npc", ...)` call — no error, the label just never draws, because `ggplot2::annotate()` doesn't know the `"text_npc"` geom that `ggpp::annotate()` (loaded via `ggpmisc`) provides. This meant the CLoD/CLoQ labels on the standard-curve panel were missing from every rendered `_summary_results.pdf` this repo has ever produced — confirmed on `examples/example_run/`'s own committed output. The R²/equation labels on the same panel were unaffected, since those come from `stat_poly_eq()`, a different code path that doesn't go through the masked `annotate()`. Fixed by preferring `ggpp::annotate` instead; all six examples re-run and confirmed to produce identical back-calculated values (including `example_run_format_consistency/`'s cross-format audit, which still passes), with the CLoD/CLoQ text now actually visible. Found while building a sibling package (liveWellR) that reused this same `conflicts_prefer` pattern.
- `examples/ELISA_assay_metadata.csv`: an unescaped comma inside the `kit` field (`Generic sandwich ELISA (colorimetric, TMB substrate)`) silently shifted every column after it, so `read_type` read back as the literal string `"200"` instead of `"Absorbance"` — with only a terse `readr` "parsing issues" warning, no error, as a clue. Found while adding the `read_type`-driven absorbance label formatting above, since it depends on `read_type` actually containing `"absorbance"`. Fixed by quoting the field.
...
- `examples/example_run_amplexred/` and `examples/example_run_generic_excel/` now each suppress one replicate well (`A6`) whose raw signal is a genuine outlier from its sibling replicates (near-background fluorescence for Amplex Red; well above the top ELISA standard for generic-Excel, edited identically in both the long- and plate-format raw files) — illustrating a realistic reason for suppression, to demonstrate the new signal-heatmap strikethrough above, rather than an arbitrarily-flagged normal replicate. Back-calculated sample values are unchanged from before, since the suppressed replicate was already excluded from averaging.
