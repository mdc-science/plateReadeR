...
# BioTek Synergy 2 exports genuine xlsx files — read directly with read_excel.
df_raw <- read_excel(
  here("experimental_data", "raw_data", datafile),
  col_names = FALSE
)

# Use the datafile name as the source identifier (analogous to original_file in SpectraMax)
original_file <- datafile

# Extract list of recorded wavelengths from the row containing "Wavelengths:"
wl_row <- which(str_detect(as.character(df_raw[[2]]), "Wavelengths:"))
wavelength_list <- df_raw[wl_row, 2] |>
  as.character() |>
  str_extract_all("[0-9]+") |>
  unlist() |>
  as.numeric() |>
...
# Synergy 2 grid format: data is laid out as an 8-row (A-H) × 12-col matrix.
# Col 2 holds the row letter; cols 3-14 hold the 12 column absorbance values.
# Each wavelength produces one grid of 8 consecutive rows.
# Detect all grid rows: col 2 is A-H AND col 3 is numeric.
grid_rows_all <- which(
  as.character(df_raw[[2]]) %in% LETTERS[1:8] &
  !is.na(suppressWarnings(as.numeric(as.character(df_raw[[3]]))))
)

# Split into groups of 8 — one group per wavelength, in order
plate_data_grids <- split(grid_rows_all, ceiling(seq_along(grid_rows_all) / 8))

positions <- generate_positions()

dataframes <- list()

for (i in seq_along(wavelength_list)) {
  wl      <- wavelength_list[i]
  df_name <- paste0("df_", wl)

  grid <- plate_data_grids[[i]]
  absorbance <- unlist(lapply(grid, \(r) as.numeric(as.character(df_raw[r, 3:14]))))
...
# Clean up workspace
rm(df_name, i, wl, absorbance, df, positions, grid_rows_all, plate_data_grids)
